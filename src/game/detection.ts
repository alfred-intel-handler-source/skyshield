/**
 * SKYSHIELD detection simulation — ported from backend/app/detection.py
...
export class SensorSimulator {
  ...
  detect_rf(drone: DroneState, sensor: SensorConfig): RfReading | null {
    ...
    const isShinobi =
      sensor.id.toLowerCase().includes('shinobi') ||
      sensor.name.toLowerCase().includes('shinobi');
    if (isShinobi) {

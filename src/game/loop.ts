...
import { updateShinobiDrone } from './shinobi.js';
...
export function tickPassiveJamming(gs: GameState, elapsed: number): Msg[] {
  ...
  for (const effState of gs.effector_states) {
    ...
    for (let i = 0; i < gs.drones.length; i++) {
      ...
      if (drone.shinobi_cm_active) continue;
...
export function tickDrones(gs: GameState, elapsed: number): Msg[] {
  ...
  for (let i = 0; i < gs.drones.length; i++) {
    ...
    // SHINOBI CM active
    if (gs.drones[i].shinobi_cm_active) {
      const [updated, nevents] = updateShinobiDrone(gs.drones[i], gs.tick_rate, elapsed);
      gs.drones[i] = updated;
      events.push(...nevents);
      continue;
    }
...
function _runSensorsForDrone(gs: GameState, i: number, elapsed: number): Msg[] {
  ...
  // SHINOBI RF data
  const shinobiUpdates: Partial<DroneState> = {};
  for (const reading of readings) {
    const r = reading as unknown as Record<string, unknown>;
    if (r.is_shinobi) {
      shinobiUpdates.frequency_band = r.frequency_band as string;
      shinobiUpdates.downlink_detected = r.downlink_detected as boolean;
      shinobiUpdates.uplink_detected = r.uplink_detected as boolean;
    }
  }
  if (Object.keys(shinobiUpdates).length > 0) {
    gs.drones[i] = { ...gs.drones[i], ...shinobiUpdates };
    drone = gs.drones[i];
  }
